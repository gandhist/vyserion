...
$(document).ajaxStart(function () {
    Pace.restart()
            ...
            $(document).ready(function () {
                ...
                $("#master_vehicle").addClass("active");
...
$(document).ready(function() {
  $(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('vehicle/vm_list')?>",
            "type": "POST",
            "data": function (data) {
    ...
    $('#f_code_unit').bind('keyup blur',function(){
        var regillegal = /[/]/ // karakter yang tidak di izinkan di letakan di regex
        if (regillegal.test($('#f_code_unit').val())) {
            alert('Symbol " / " backslash is not allowed!!');
        }
        $('#f_code_unit').val( $('#f_code_unit').val().replace(/[^A-Za-z0-9-.]/g,'')); // karakter yang di izinkan
    });
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('vehicle/vm_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            //===== left side is input name and then right side is field name
            $('[name="f_id_vehicle"]').val(data.id_vehicle);
            $('[name="f_code_unit"]').val(data.code_unit);
            $('[name="f_nopol"]').val(data.nomor_plat);
            $('[name="f_nap"]').val(data.nap);
            $('[name="f_empcode"]').val(data.pic_code).trigger("change");
            $('[name="f_tahun"]').val(data.year);
            $('[name="f_ownership"]').val(data.ownership).trigger("change");
            $('[name="f_status_unit"]').val(data.status_unit).trigger("change");
            $('[name="f_group"]').val(data.groups).trigger("change");
            $('[name="f_work_unit"]').val(data.work_unit).trigger("change");
            $('[name="f_jenis_type"]').val(data.type);
            $('[name="f_model"]').val(data.model).trigger("change");
            $('[name="f_manufacturer"]').val(data.manufacturer);
            $('[name="f_no_rangka"]').val(data.no_frame);
            $('[name="f_no_mesin"]').val(data.no_machine);
            $('[name="f_sn"]').val(data.serial_number);
            $('[name="f_engine_model"]').val(data.engine_model);
            $('[name="f_engine_sn"]').val(data.engine_sn);
            $('[name="f_silinder"]').val(data.cylinder);
            $('[name="f_tanggal_terima"]').val(data.date_receive);
            $('[name="f_pm_service"]').val(data.kelipatan_pm);
            $('[name="f_remarks"]').val(data.remarks);
            $('input[name=f_active][value="' + data.active + '"]').prop('checked', true).trigger('change');
            $('input[name=f_to_rpt_bd][value="' + data.to_rpt_bd + '"]').prop('checked', true).trigger('change');
            $('input[name=f_to_rpt_hm][value="' + data.to_rpt_hm + '"]').prop('checked', true).trigger('change');
  ...
  if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('vehicle/vm_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function sav()
{
    ...
    if (save_method == 'add') {
        ...
        url = "<?php echo site_url('vehicle/vm_add') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
        ...
        dataType: "JSON",
               ...
               if(data.status) //if success close modal and reload ajax table
            {
              alert('data berhasil disimpan');
                reset_form(); // will reset and reload datatable
                //$("#f_date_attd").focus();
            }
            else
            {
    ...
    else 
    {
        ...
        url = "<?php echo site_url('vehicle/vm_update')?>";
        var formData = new FormData($('#form')[0]);
    $.ajax({
        url : url,
        type: "POST",
...
function reset_form()
{
    //alert('fuckyou');
        ...
        $("#f_ownership").val(null).trigger('change');
        $("#f_status_unit").val(null).trigger('change');
        $("#f_group").val(null).trigger('change');
        $("#f_work_unit").val(null).trigger('change');
        $("#f_model").val(null).trigger('change');
        $("#btnSave").attr("onclick","add()");
        table.ajax.reload();  //just reload table
...
{
    if(confirm('Are you sure to inactive this vehicle?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('vehicle/inactive')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
