...
$(document).ajaxStart(function () {
    Pace.restart()
            ...
            $(document).ready(function () {
                ...
                $("#filter_month").val(moment().format('M')).change('trigger'); // default value for month
            });
                // untuk setting sidebar active
                $("#vehicle").addClass("treeview active");
                $("#rpt_vehicle").addClass("treeview active");
                $("#rpt_veh_services").addClass("active");
...
$(document).ready(function() {
    ...
    $('#filter_month').bind('keyup blur',function(){
        var regillegal = /[A-Za-z./-]/ // karakter yang tidak di izinkan di letakan di regex
        if (regillegal.test($('#filter_month').val())) {
            alert('invalid characters, use only 0-9!!');
        }
        $('#filter_month').val( $('#filter_month').val().replace(/[^0-9]/g,'')); // karakter yang di izinkan
    });
