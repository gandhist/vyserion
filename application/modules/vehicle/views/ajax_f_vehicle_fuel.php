...
/*fungsi jquery untuk mendapatkan total KM pada shift1*/
  ...
  /*VALIDASI ONLY CHAR ALLOWED*/
    $('#f_no_voucher').bind('keyup blur',function(){
        var regillegal = /[/A-Za-z]/ // karakter yang tidak di izinkan di letakan di regex
        if (regillegal.test($('#f_no_voucher').val())) {
            alert('Invalid Characters is not allowed!!');
        }
        $('#f_no_voucher').val( $('#f_no_voucher').val().replace(/[^0-9-]/g,'')); // karakter yang di izinkan
    });
    $('#f_qty').bind('keyup blur',function(){
        var regillegal = /[/A-Za-z]/ // karakter yang tidak di izinkan di letakan di regex
        if (regillegal.test($('#f_qty').val())) {
            alert('Invalid Characters is not allowed!!');
            validasi_shift_satu();
        }
        $('#f_qty').val( $('#f_qty').val().replace(/[^0-9]/g,'')); // karakter yang di izinkan
    });
    $('#f_hmkm').bind('keyup blur',function(){
        var regillegal = /[/A-Za-z]/ // karakter yang tidak di izinkan di letakan di regex
        if (regillegal.test($('#f_hmkm').val())) {
            alert('Invalid Characters is not allowed!!');
        }
        $('#f_hmkm').val( $('#f_hmkm').val().replace(/[^0-9]/g,'')); // karakter yang di izinkan
    });
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('vehicle/veh_fuel_list')?>",
            "type": "POST",
            "data": function (data) {
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('vehicle/veh_fuel_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            //===== left side is input name and then right side is field name
            $('[name="f_id_vehicle_fuel"]').val(data.id);
            $('[name="f_no_voucher"]').val(data.no_voucher);
            $('[name="f_date"]').val(data.tanggal_pengisian);
            $('[name="f_nap"]').val(data.nap).trigger("change");
            $('[name="f_qty"]').val(data.qty);
            $('[name="f_hmkm"]').val(data.hmkm);
            $('[name="f_fuelman"]').val(data.fuelman).trigger('change');
            $('[name="f_driver"]').val(data.driver).trigger('change');
  ...
  if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('vehicle/veh_fuel_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function sav()
{
    ...
    if (save_method == 'add') {
        ...
        url = "<?php echo site_url('vehicle/vehicle_fuel_add') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
        ...
        dataType: "JSON",
               ...
               if(data.status) //if success close modal and reload ajax table
            {
              //alert('data berhasil disimpan');
                $('#info_saved').show();
                setInterval(function(){ $('#info_saved').fadeOut(); }, 3000);
    ...
    else 
    {
        ...
        url = "<?php echo site_url('vehicle/veh_fuel_update')?>";
        var formData = new FormData($('#form')[0]);
    $.ajax({
        url : url,
        type: "POST",
...
<script>
  $(function () {
    ...
    //$("#f_hmkm").inputmask('000.000.000.000.000.0', {reverse: true});
    //$("#f_hmkm").inputmask('0.0', {reverse: true});
