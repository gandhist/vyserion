...
class Monitoring_p2h_model extends CI_Model {
	...
	var $column_order = array('d.nomor_plat'); //set column field database for datatable orderable
	var $column_search = array('d.nomor_plat','b.nap','d.code_unit'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	var $order = array('f.nap' => 'asc'); // default order 
	...
	{
        /*SET @h_min_satu = CURDATE() - INTERVAL 1 DAY;
SET @h_min_empat = CURDATE() - INTERVAL 4 DAY;
    SELECT f.* FROM
    ...
    /*        
        ...
        $this->db->select("f.*");
        $this->db->from('(SELECT COUNT(a.nap) total_tidakp2h, a.nap, c.nomor_plat, get_empname(c.pic_code) pic, c.code_unit, b.last_km, b.tanggal AS last_date FROM 
    (SELECT nap,kondisi, tanggal FROM vehicle_daily_op WHERE tanggal BETWEEN @h_min_empat AND @h_min_satu) a
    RIGHT JOIN 
    (SELECT b.tanggal, b.nap,
        CASE
    ...
    ON a.nap = b.nap
    LEFT JOIN vehicle_master c ON a.nap = c.nap
    WHERE a.kondisi = 5 GROUP BY a.nap) f');
        $this->db->where('f.total_tidakp2h >=  3 and f.nap not in ("97-68","97-43") ');
