...
class Vehicle_monitoring_model extends CI_Model {
	...
	var $table = 'document_vehicle';
	var $column_order = array(null,'b.nomor_plat','a.nap', 'a.doc_type', 'a.valid_until', 'a.due_date','a.remarks', null, null); //set column field database for datatable orderable
	var $column_search = array('a.doc_no', 'a.nap', 'a.doc_type', 'a.valid_until', 'a.remarks','b.nomor_plat'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	var $order = array('valid_until' => 'asc'); // default order 
	...
	private function _get_datatables_query()
	{
        ...
        /*SELECT code_unit, doc_type, valid_until, due_date FROM
(SELECT id_document, doc_no, code_unit, doc_type, valid_until, DATEDIFF(valid_until, CURDATE()) AS due_date, STATUS, CASE
  ...
  WHERE a.show_notif = 1*/
        $this->db->select('a.id_document, a.doc_no, a.nap, b.nomor_plat, b.code_unit, a.doc_type,a.valid_until,a.due_date,a.remarks,a.status,');
        $this->db->from('(SELECT id_document,
  doc_no,
  nap,
  doc_type,
  valid_until,
  DATEDIFF(valid_until, CURDATE()) AS due_date,
  CASE
  WHEN DATEDIFF(valid_until, CURDATE()) <= 60 THEN "1"
  ELSE "0"
  END AS show_notif,
  ...
  mail_stnk,
  mail_keur,
  mail_iu from document_vehicle where status != "closed") a');
        $this->db->join('vehicle_master b','a.nap = b.nap','right');
        /*$this->db->from($this->table);*/
        $this->db->where('a.show_notif','1');
        $this->db->order_by('a.valid_until','asc');
	...
	{
		$this->db->where('id_document', $id);
		$this->db->delete('document_vehicle');
	}
