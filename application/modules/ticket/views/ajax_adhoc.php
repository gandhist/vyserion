...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        /*memberikan warna tertentu ke data yang memnenuhi kondisi, css highlight di dapat dari view title*/
        "createdRow": function(row, data, index){
            if (data[3] == "D01") {
                console.log(data[3]);
                $('td',row).addClass('highlight');
                //$('td',row).eq(3).addClass('highlight');
                //$('span',row).addClass('label label-warning');
                $('td',row).eq(3).append('<span class="label label-warning"> asd</span>');
            }
        },
        "createdCell": function (td, cellData, rowData, row, col) {
              if (data[3] == "D03") {
                $('td',col).append('<span class="label label-warning"> asd</span>');
              }
            },
        //"scrollX": "200px",
        // Load data for the table's content from an Ajax source
        "ajax": {
            "url": "<?php echo site_url('payroll/adhoc_list')?>",
            "type": "POST",
            "data": function (data) {
...
function add_person()
{
    ...
    $('.modal-title').text('Input ADHOC'); // Set Title to Bootstrap modal title
    ...
    $("#f_adhoc_code").val(null).trigger('change');
...
{
    alert('button upload a file has clicked');
}
...
function edit_emp_attribute(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('payroll/adhoc_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="f_adhoc_date"]').val(data.adhoc_date);
            $('[name="f_adhoc_code"]').val(data.allowded_code).trigger("change");
            $('[name="f_id_adhoc"]').val(data.id_adhoc);
            $('[name="f_amount"]').val(data.amount);
...
function sav()
{
    ...
    if (save_method == 'add') {
        url = "<?php echo site_url('payroll/adhoc_add') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
    ...
    {
        url = "<?php echo site_url('payroll/adhoc_update')?>";
        var formData = new FormData($('#form')[0]);
    $.ajax({
        url : url,
        type: "POST",
...
// ajax jquery upload file adhoc
$('#loading_import').hide();
    $('#btn_import').click(function(){
        var url = "<?php echo site_url('payroll/import_adhoc')?>";
...
function delete_data(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('payroll/adhoc_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
