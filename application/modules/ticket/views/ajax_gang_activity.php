...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('ticket/ajax_list')?>",
            "type": "POST",
            "data": function (data) {
...
function edit_emp_attribute(id)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="f_request_date"]').val(data.request_date);
            $('[name="f_request_by"]').val(data.request_by);
            $('[name="f_id"]').val(data.id);
            $('[name="f_problem"]').val(data.problem);
  ...
  if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('ticket/emp_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function sav()
{
    ...
    if (save_method == 'add') {
        url = "<?php echo site_url('ticket/emp_add') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
        ...
        dataType: "JSON",
            ...
            {
              alert('data berhasil disimpan cuy');
                reload_table();
                $('#form')[0].reset();
                //$("#f_date_attd").focus();
            }
    ...
    {
        url = "<?php echo site_url('ticket/emp_update')?>";
        var formData = new FormData($('#form')[0]);
    $.ajax({
        url : url,
        type: "POST",
