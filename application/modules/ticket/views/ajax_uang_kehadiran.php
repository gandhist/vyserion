...
<script>
            $(document).ready(function () {
                ...
                $("#setup_payroll").addClass("treeview active");
                $("#uang_ump").addClass("active");
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('payroll/ump_list')?>",
            "type": "POST",
            "data": function (data) {
...
function edit_emp_attribute(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('payroll/ump_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            //===== left side is input name and then right side is field name
            $('[name="f_date"]').val(data.effective_date);
            $('[name="f_id"]').val(data.id);
            $('[name="f_rate"]').val(data.rate);
...
function sav()
{
    ...
    if (save_method == 'add') {
        url = "<?php echo site_url('payroll/add_ump') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
    ...
    {
        url = "<?php echo site_url('payroll/ump_update')?>";
        var formData = new FormData($('#form')[0]);
    $.ajax({
        url : url,
        type: "POST",
