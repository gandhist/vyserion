...
class Hr_report extends CI_Model {
    ...
    var $column_order = array('a.empcode','a.empname','a.departement','a.position','a.companybegin',null); //set column field database for datatable orderable
    var $column_search = array('a.empcode','a.empname','a.departement','a.position','a.companybegin'); //set column field database for datatable searchable just firstname , lastname , address are searchable
...
SELECT 
  ...
  c.dept_desc,
  CASE
    WHEN a.employeetype = "kt" THEN "Karyawan Tetap"
    ELSE a.employeetype
...
*/
        ...
        {
            $this->db->where('a.departement', $this->input->post('filter_dept'));
        }
        if($this->input->post('filter_position'))
        {
            $this->db->where('a.position', $this->input->post('filter_position'));
        }
        if($this->input->post('filter_emp_status'))
        {
            $this->db->where('a.employeetype', $this->input->post('filter_emp_status'));
        }
        if($this->input->post('filter_start_date') != '' && $this->input->post('filter_end_date') != '' || $this->input->post('filter_start_date') != null)
        {
        $this->db->where('a.hire_date BETWEEN "'.$this->input->post('filter_start_date').'" AND "'.$this->input->post('filter_end_date').'"');
  ...
  a.sim_valid");
        ...
        $this->db->join('master_position b','a.position = b.id_position','left');
        $this->db->join('master_dept c','a.departement = c.departement','left');
        $this->db->where('a.is_deleted != "1" and a.empcode not in (SELECT empcode FROM hr_termination WHERE dateterminate < CURRENT_DATE() - INTERVAL 26 DAY) and a.site_id = 2001');
