...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('hr/hr/tax_list')?>",
            "type": "POST"
        },
...
function add()
{
    ...
    $('.modal-title').text('Add Tax Status'); // Set Title to Bootstrap modal title
   ...
   /* $('#f_input_date').val('<?php echo date("Y-m-d h:i:s"); ?>');*/
    ...
    $("#f_family_tax_status").val(null).trigger('change');
    $("#f_rice_tax_status").val(null).trigger('change');
...
/*          var date = new Array();
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="f_family_tax_status"]').val(data.familystatustax).trigger('change');
            $('[name="f_rice_tax_status"]').val(data.familystatusrice).trigger('change');   
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo site_url('hr/hr/tax_status_add')?>";
    } else {
      time_stamp_update(); // fungsi timestamp
        url = "<?php echo site_url('hr/hr/tax_update')?>";
    }
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Tax Status</h3>
            </div>
      ...
      <table cellpadding="50" cellspacing="50">
        ...
        <input type="text" style="display:none;" name="f_update_date" id="f_update_date" value="">
         <input type="text" style="display:none;" name="f_empcode_4_update" id="f_empcode_4_update" value="">
                              ...
                              <tr>
                                <td>Family Status Tax </td>
                                <td>
                                  <select class="form-control select2" style="width: 100%;" id="f_family_tax_status" name="f_family_tax_status">
                                      <option value="K/0">K/0</option> <!-- menikah 0 tanggungan -->
                                      <option value="K/1">K/1</option>
                                      <option value="K/2">K/2</option>
                                      <option value="K/3">K/3</option>
                                      <option value="K/4">K/4</option>
                                      <option value="TK/0">TK/0</option> <!-- tidak menikah 0 tanggungan -->
                              ...
                              <tr>
                                <td>Rice Status Tax </td>
                                <td>
                                  <select class="form-control select2" style="width: 100%;" id="f_rice_tax_status" name="f_rice_tax_status">
                                      <option value="K/0">K/0</option> <!-- menikah 0 tanggungan -->
                                      <option value="K/1">K/1</option>
                                      <option value="K/2">K/2</option>
                                      <option value="K/3">K/3</option>
