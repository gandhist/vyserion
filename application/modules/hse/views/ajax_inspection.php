...
$(document).ajaxStart(function () {
    Pace.restart()
            ...
            $(document).ready(function () {
                ...
                $("#hse").addClass("treeview active");
                $("#input_hse").addClass("treeview active");
                $("#hse_insepction").addClass("active");
...
$(document).ready(function() {
    ...
    $("#f_category").val(null).trigger('change');
    $("#f_reportby").val(null).trigger('change');
    $("#f_dept").val(null).trigger('change');
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('hse/inspection_list')?>",
            "type": "POST",
            "data": function (data) {
            ...
            for ( var i=data.start, ien=data.start+data.length ; i<ien ; i++ ) {
                ...
                data.filter_desc = $('#filter_desc').val();
                data.filter_act = $('#filter_act').val();
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('hse/edit_inspection')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            //===== left side is input name and then right side is field name
            $('[name="f_date"]').val(data.date);
            $('[name="f_desc"]').val(data.desc);
            $('[name="f_nap"]').val(data.nap).trigger("change");
            $('[name="f_category"]').val(data.category).trigger("change");
            $('[name="f_reportby"]').val(data.report_by).trigger("change");
            $('[name="f_dept"]').val(data.dic).trigger("change");
            $('[name="f_act"]').val(data.action);
            $('[name="f_comp"]').val(data.company);
            $('[name="f_status"]').val(data.status).trigger("change");
            $('[name="f_prior"]').val(data.priority);
            $('[name="f_id"]').val(data.id);
  ...
  if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('hse/inspection_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function sav()
{
    ...
    if (save_method == 'add') {
        ...
        url = "<?php echo site_url('hse/add_inspection') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
        ...
        dataType: "JSON",
               ...
               if(data.status) //if success close modal and reload ajax table
            {
              ...
              $('[name="f_desc"]').val(null);
                $('[name="f_nap"]').val(null).trigger("change");
                $('[name="f_category"]').val(null).trigger("change");
                $('[name="f_reportby"]').val(null).trigger("change");
                $('[name="f_dept"]').val(null).trigger("change");
                $('[name="f_act"]').val(null);
                $('[name="f_comp"]').val(null);
                $('[name="f_status"]').val(null).trigger("change");
                $('[name="f_prior"]').val(null);
                table.ajax.reload(); 
    ...
    else 
    {
        ...
        url = "<?php echo site_url('hse/update_inspection')?>";
        var formData = new FormData($('#form')[0]);
    $.ajax({
        url : url,
        type: "POST",
