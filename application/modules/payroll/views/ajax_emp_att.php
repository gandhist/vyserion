...
$(document).ajaxStart(function () {
    Pace.restart()
            ...
            $(document).ready(function () {
                ...
                $("#emp_att").addClass("active");
                $('#alert_import').hide();
                $('#download_indicator').hide();
                $('#download_indicator_presensi').hide();
...
var dept; //for save method string
var table;
...
$(document).ready(function() {
    ...
    $("#f_empcode_f").val(null).trigger('change');
        ...
        $("#f_hadir").val(null).trigger('change');
        $("#f_late").val(null).trigger('change');
        $("#f_shift").val(null).trigger('change');
        $("#f_dept_download").val(null).trigger('change');
        $("#f_empcode").val(null).trigger('change');
    });
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('payroll/emp_attd_list')?>",
            "type": "POST",
            "data": function (data) {
            ...
            for ( var i=data.start, ien=data.start+data.length ; i<ien ; i++ ) {
                ...
                data.f_dept = $('#f_dept').val();
                data.f_hadir = $('#f_hadir').val();
                data.f_late = $('#f_late').val();
    ...
    $('#f_end_date').change(function(){
        ...
        var ms = moment(current_date, "DD-MM-YYYY").diff(moment(end_date, "DD-MM-YYYY"));
        var dif = moment.duration(ms);
        var variance = dif.asDays();
        console.log(variance);
        if (variance < 0) {
            $('#btnSave').attr('disabled',true);
            $('#f_end_date').parent().parent().addClass('has-error'); //select parent twice to select div form-group class and add has-error class
            $('#f_end_date').next().text("End date must until "+current_date);
        }
        else {
            var ms_st = moment(start_date,"DD-MM-YYYY").diff(moment(end_date, "DD-MM-YYYY"));
            var div = moment.duration(ms_st);
            var beda = div.asDays();
            if (beda > 0 ) {
                $('#btnSave').attr('disabled',true);
                $('#f_end_date').parent().parent().addClass('has-error'); //select parent twice to select div form-group class and add has-error class
                $('#f_end_date').next().text("End date must bigger than "+start_date);
            }
            else {

            $('#btnSave').attr('disabled',false);
            $('#f_end_date').parent().parent().removeClass('has-error'); //select parent twice to select div form-group class and add has-error class
            $('#f_end_date').next().text("");
            }
        }
    });
...
function add()
{
    ...
    time_stamp.val(time);
    save_method = 'add';
    dept = $('#f_dept_download').val();
    $('#download_indicator_presensi').show();
...
function import_absen()
{
    //alert("fuck");
    $('#download_indicator').show();

    $.ajax({
        url : "<?php echo site_url('payroll/import_from_faceprint_server') ?>",
        type : "GET",
        dataType : "JSON",
        success : function(data)
        {
            console.log(data);
            alert("import data from machine face print to Database successfully!!");
            $('#alert_import').show();
            $('#download_indicator').hide();
            $('#success_message').text(data.hasil_copy);
        },
        error : function(jqXHR, textStatus, errorThrown)
        {
            $('#download_indicator').hide();
...
function sav()
{
    ...
    if (save_method == 'add') {
        ...
        if (dept == '8') {
        url = "<?php echo site_url('payroll/download_data_presensi_pro') ?>"; 
        }
        else {
        url = "<?php echo site_url('payroll/download_data_presensi') ?>";
        }
        $.ajax({
            url: url,
            type: 'POST',
