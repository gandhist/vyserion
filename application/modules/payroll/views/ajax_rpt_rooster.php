...
$(document).ajaxStart(function () {
    Pace.restart()
        ...
        $("#periode").val(moment().format('M')).change('trigger'); // default value for month

                
            });
                // untuk setting sidebar active
                $("#prod").addClass("treeview active");
                $("#report_prod").addClass("treeview active");
                $("#rpt_rooster").addClass("active");
...
$(document).ready(function() {
    $('#btn-filter').click(function(){ //button filter event click
       //alert('fucking hero');
        ...
        $('#filter_year').val();
        $.ajax({
            url : "<?php echo site_url('payroll/_validatee')?>",
            type: 'POST',
            dataType : "json",
    ...
    $('#f_rpt').on('select2:select', function(){
    var empcode = $('#f_rpt').val();
      switch (empcode) {
            case "roster":
                $('#form_roster').attr('action','<?php echo base_url(); ?>production/export_rooster_ex_pro_rev1');
                hide_start_end_date();
                break;
            case "monthly":
                $('#form_roster').attr('action','<?php echo base_url(); ?>payroll/export_monthly_absensi');
                hide_start_end_date();
                break;
            case "insentive":
                $('#form_roster').attr('action','<?php echo base_url(); ?>payroll/export_monthly_absensi_insentive');
                show_start_end_date();               
            break;
      }
    });

    $('#btn-filtear').click(function(){ //button filter event click
