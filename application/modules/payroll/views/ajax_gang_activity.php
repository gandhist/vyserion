...
$(document).ready(function() {
        ...
        $("#f_mass_update").val(null).trigger('change');
        $("#f_mass_update_f").val(null).trigger('change');
        $("#f_gang").val(null).trigger('change');
        $('#f_shift').val(null).trigger('change');
        $('#f_attd_code_f').val(null).trigger('change');
        $('#f_work_shift').val(null).trigger('change');
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            ...
            for ( var i=data.start, ien=data.start+data.length ; i<ien ; i++ ) {
                ...
                data.f_mass_update_f = $('#f_mass_update_f').val();
                data.f_shift = $('#f_shift').val();
                data.filter_wh = $('#filter_wh').val();
                data.f_attd_code_f = $('#f_attd_code_f').val();
    ...
    /*validasi komponen gaji workhours dan overtime*/
     $('#f_wh').bind('keyup blur',function(){
        ...
        if (regillegal.test($('#f_wh').val())) {
            alert('input bilangan bulat saja!!');
        }
    ...
    // dapatkan gang by empcode
    $('#f_empcode').on('select2:select', function(){
        var empcode = $('#f_empcode').val();
        //alert(empcode);
        $.ajax({
        url : "<?php echo site_url('payroll/get_gang')?>/" + empcode,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            //===== left side is input name and then right side is field name
            $('#f_gang').val(data[0].id_gang).trigger("change");
            $
            console.log(data);
            //$('[name="f_gang"]').val(data.id_gang).trigger("change");
        
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
            alert('Error get gang, maybe its not set on gang master.');
        }
    });
    });
    ...
    $('#f_attd_code').on('select2:select', function(){
        ...
        if (attd_code == "attd004" || attd_code == "attd005" || attd_code == "attd006" || attd_code == "attd007" || attd_code == "attd008" || attd_code == "attd009" ) {
            //alert('holycrap its izin and alpa')
            $('#f_wh').attr('readonly',true);
            $('#f_wh').val(0);
            $('#f_shw').val(0);
            $('#f_whtl').val(0);
            $('#f_off_day').val(0);
            ...
            if (attd_code == "attd006") {
                $('#f_work_shift').val("OFF DAY").trigger('change');
            }
            else{
                $('#f_work_shift').val(null).trigger('change');
            }
        ...
        else if(attd_code == "attd001" || attd_code == "attd002" || attd_code == "attd003")
        {
            $('#f_wh').attr('readonly',false);
            if (work_hour > 0 ) {
...
function edit_emp_attribute(id)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="f_work_shift"]').val(data.shift).trigger("change");
            $('[name="f_mass_update"]').val(data.is_mass_update).trigger("change");
            $('[name="f_gang"]').val(data.id_gang).trigger("change");
...
function ajax_copy()
{
    var formData = new FormData($('#form_copy')[0]);
    var url = "<?php echo site_url('payroll/test_copy_data') ?>";
    $.ajax({
        url: url,
        type: 'POST',
        data: formData,
        ...
        error : function(jqXHR, textStatus, errorThrown)
        {
            alert('Data master belum di input');
            //console.log(formData);
        }
    });
}

function ajax_copy_all()
{
    var formData = new FormData($('#form_copy_all')[0]);
    var url = "<?php echo site_url('payroll/get_empcode_last_period') ?>";
    $.ajax({
        url: url,
        type: 'POST',
        data: formData,
        ...
        {
            if (data.status) {
            alert('Berhasil Copy Data Bulan Sebelumnya');
            }
            else
            {
            for (var i = 0; i < data.inputerror.length; i++) 
...
function sav_mass()
{
    ...
    if (save_method == 'add') {
        ...
        time_stamp_input();
        url = "<?php echo site_url('payroll/mass_add') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
    ...
    $('#f_wh').attr('readonly',false);
               ...
               if(data.status) //if success close modal and reload ajax table
            {
              //alert('data berhasil disimpan');
                 ...
                 $('#saved_message').add().text(data.inserted+' Data Inserted');
                setInterval(function(){ $('#data_updated').fadeOut(); }, 3000);
                reload_table();
                $('#form')[0].reset();
                $("#f_date_attd").focus();
                $("#f_empcode").val(null).trigger('change');
    ...
    {
    time_stamp_update();
        url = "<?php echo site_url('payroll/mass_update')?>";
        var formData = new FormData($('#form')[0]);
    $.ajax({
        url : url,
        type: "POST",
    ...
    $('#f_wh').attr('readonly',false);
            ...
            if(data.status) //if success close modal and reload ajax table
            {
                ...
                */
                ...
                $('#data_updated').show();
                 //$('#saved_message').remove();
                 $('#saved_message').add().text(data.inserted+' Data Upadated');
                setInterval(function(){ $('#data_updated').fadeOut(); }, 3000);
                //alert(save_method);
                reload_table();
                $('#form')[0].reset();
