...
$(document).ajaxStart(function () {
    Pace.restart()
            ...
            $(document).ready(function () {
                ...
                $("#process_payroll").addClass("treeview active");
                $("#closing_payroll").addClass("active");
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('payroll/closing_list')?>",
            "type": "POST",
            "data": function (data) {
            ...
            for ( var i=data.start, ien=data.start+data.length ; i<ien ; i++ ) {
                ...
                data.filter_period = $('#filter_period').val();
...
function sav()
{
    ...
    if (save_method == 'add') {
        ...
        time_stamp_input();
        url = "<?php echo site_url('payroll/do_closing') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
        ...
        dataType: "JSON",
               ...
               if(data.status) //if success close modal and reload ajax table
            {
                ...
                if (data.error_string == "Data Sudah Terinput") {
                    for (var i = 0; i < data.inputerror.length; i++) 
                {
                    $('[name="'+data.inputerror[i]+'"]').parent().parent().addClass('has-error'); //select parent twice to select div form-group class and add has-error class
                    $('[name="'+data.inputerror[i]+'"]').next().text(data.error_string[i]); //select span help-block class set text error string
                }
                // konfirmasi jika data di delete
                delete_emp_attribute(period, year); 
                add_closing(period, year); 
                }
                else{
                    alert('Proses Closing Berhasil');
                    $(this).parent().parent().removeClass('has-error');
                    $(this).next().empty();
                    reload_table();
            ...
            //$('#btnSave').text('Closi'); //change button text
...
function delete_emp_attribute(period, year)
{
    if(confirm('Are you sure to reclosing this period?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('payroll/delete_all_before_closing')?>/"+period+"/"+year,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function add_closing(period, year)
{
    $.ajax({
        url :  "<?php echo site_url('payroll/add_closing')?>/"+period+"/"+year, 
        type : "POST",
        dataType : "JSON",
        success: function(data)
        {
            alert('data berhasil di closing');  
            $('[name="'+data.inputerror[i]+'"]').parent().parent().removeClass('has-error');
            $('[name="'+data.inputerror[i]+'"]').next().empty();
        },
        error : function(jqXHR, textStatus, errorThrown)
        {
            alert('error get data from ajax');
