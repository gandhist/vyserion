...
$(document).ajaxStart(function () {
    Pace.restart()
            ...
            $(document).ready(function () {
                ...
                $("#setup_prod").addClass("treeview active");
                $("#gang_master").addClass("active");
...
$(document).ready(function() {
   ...
   $(document).ready(function () {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('production/gang_master_list')?>",
            "type": "POST",
            "data": function (data) {
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('production/gang_master_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            //===== left side is input name and then right side is field name
            $('[name="f_id_gang_master"]').val(data.empcode);
...
function sav()
{
    ...
    if (save_method == 'add') {
        ...
        url = "<?php echo site_url('production/gang_master_add') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
    ...
    else 
    {
        ...
        url = "<?php echo site_url('production/gang_master_update')?>";
        var formData = new FormData($('#form')[0]);
    $.ajax({
        url : url,
        type: "POST",
...
{
        time_stamp_update();
    if(confirm('Are you sure to inactive this employee?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('production/inactive_gang_master')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
