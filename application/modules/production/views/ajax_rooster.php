...
$(document).ajaxStart(function () {
    ...
    $('#input_indicator').hide(); // sembunyikan loading indicator

                                 // set form to null values
   /*$("#f_empcode_f").val(null).trigger('change');
        $("#f_gang_f").val(null).trigger('change');
        $("#f_period_f").val(null).trigger('change');
        $("#f_year_f").val(null).trigger('change');*/
  $("#f_empcode").val(null).trigger('change');
        $("#f_gang").val(null).trigger('change');
                $(".select2").select2({
...
$(document).ready(function() {
   ...
   $(document).ready(function () {
            ...
            $("#f_year_f").val(null).trigger('change');
            $("#f_empcode").val(null).trigger('change');
            $("#f_period").val(null).trigger('change');
            $("#f_year").val(null).trigger('change');
            $("#f_begin_work").val(null).trigger('change');
            $('#f_div_day').hide();
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('production/rooster_list')?>",
            "type": "POST",
            "data": function (data) {
            ...
            for ( var i=data.start, ien=data.start+data.length ; i<ien ; i++ ) {
                ...
                data.f_period_f = $('#f_period_f').val();
                data.f_year_f = $('#f_year_f').val();
    ...
    $('#f_begin_work').change(function(){
        var begin_work = $('#f_begin_work').val();
        if (begin_work == 612 || begin_work == 621 || begin_work == 601 || begin_work == 602) {
            $('#f_div_day').show();
          
            //alert("ini 6 pagi dan malam");
        }
        else{
...
// ketika tombol inactive no di pilih maka akan tampil field inactive date
...
function add()
{
    ...
    $('#input_indicator').show(); // menampilkan loading indicator
    save_method = 'add';
    $('#f_input_by').val('<?php echo $username; ?>');
    time_stamp_input();
    sav();
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('production/rooster_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            //===== left side is input name and then right side is field name
            $('[name="f_id_rooster"]').val(data.id_rooster_sk);
            $('[name="f_empcode"]').val(data.empcode).trigger('change');
            $('[name="f_period"]').val(data.period).trigger('change');
            $('[name="f_year"]').val(data.year).trigger('change');
            $('[name="f_begin_work"]').val(data.code_begin).trigger('change');
            $('[name="f_remarks"]').val(data.remarks);
            //$('[name="f_gang_desc"]').val(data.gang_desc);
            //$('input[name=f_active][value="' + data.active + '"]').prop('checked', true).trigger('change');
...
function sav()
{
    ...
    if (save_method == 'add') {
        ...
        url = "<?php echo site_url('production/rooster_add') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
    ...
    {
        url = "<?php echo site_url('production/rooster_update')?>";
    ...
    $('#input_indicator').hide(); // sembunyikan loading indicator
            ...
            if(data.status) //if success close modal and reload ajax table
            {
                ...
                */
                ...
                reset_form();
                /*$("#f_empcode").val(null).trigger('change');
                $("#f_ownership").val(null).trigger('change');
                $("#f_status_unit").val(null).trigger('change');*/
...
function inactive(id)
{
    ...
    time_stamp_update();
    var formData = new FormData($('#form')[0]);
    if(confirm('Are you sure to delete this employee from rooster?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('production/inactive_rooster')?>/"+id,
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
...
function rooster_edit(id)
{

    $('#modal_form_edit_rooster').modal('show'); // show bootstrap modal
    $('.modal-title').text('Edit Rooster'); // Set Title to Bootstrap modal title
    //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('production/rooster_edit')?>/" + id,
        type: "GET",
        ...
        success: function(data)
        {
            ...
            $('[name="f_begin_work"]').val(data.code_begin).trigger('change');
            $('[name="f_remarks"]').val(data.remarks);
            $('[name="f_day_1"]').val(data.a1).trigger('change');
            $('[name="f_day_2"]').val(data.a2).trigger('change');
            $('[name="f_day_3"]').val(data.a3).trigger('change');
            $('[name="f_day_4"]').val(data.a4).trigger('change');
            $('[name="f_day_5"]').val(data.a5).trigger('change');
            $('[name="f_day_6"]').val(data.a6).trigger('change');
            $('[name="f_day_7"]').val(data.a7).trigger('change');
            $('[name="f_day_8"]').val(data.a8).trigger('change');
            $('[name="f_day_9"]').val(data.a9).trigger('change');
            $('[name="f_day_10"]').val(data.a10).trigger('change');
            $('[name="f_day_11"]').val(data.a11).trigger('change');
            $('[name="f_day_12"]').val(data.a12).trigger('change');
            $('[name="f_day_13"]').val(data.a13).trigger('change');
            $('[name="f_day_14"]').val(data.a14).trigger('change');
            $('[name="f_day_15"]').val(data.a15).trigger('change');
            $('[name="f_day_16"]').val(data.a16).trigger('change');
            $('[name="f_day_17"]').val(data.a17).trigger('change');
            $('[name="f_day_18"]').val(data.a18).trigger('change');
            $('[name="f_day_19"]').val(data.a19).trigger('change');
            $('[name="f_day_20"]').val(data.a20).trigger('change');
            $('[name="f_day_21"]').val(data.a21).trigger('change');
            $('[name="f_day_22"]').val(data.a22).trigger('change');
            $('[name="f_day_23"]').val(data.a23).trigger('change');
            $('[name="f_day_24"]').val(data.a24).trigger('change');
            $('[name="f_day_25"]').val(data.a25).trigger('change');
            $('[name="f_day_26"]').val(data.a26).trigger('change');
            $('[name="f_day_27"]').val(data.a27).trigger('change');
            $('[name="f_day_28"]').val(data.a28).trigger('change');
            $('[name="f_day_29"]').val(data.a29).trigger('change');
            $('[name="f_day_30"]').val(data.a30).trigger('change');
            $('[name="f_day_31"]').val(data.a31).trigger('change');
            $('[name="f_day_32"]').val(data.a32).trigger('change');
            //$('[name="f_gang_desc"]').val(data.gang_desc);
            //$('input[name=f_active][value="' + data.active + '"]').prop('checked', true).trigger('change');
            //$('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            //$('.modal-title').text('Edit Employee Bio Data'); // Set title to Bootstrap modal title
...
function save_rooster_edit()
{
    //alert("god damn it");
    $('#f_update_by_rooster').val('<?php echo $username; ?>');
    time_stamp_update();
    var formData = new FormData($('#form_edit_rooster')[0]);
    $.ajax({
        url: "<?php echo site_url('production/rooster_update_tanggal') ?>",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
        ...
        success: function(data)
        {
                if(data.status) //if success close modal and reload ajax table
            {
                $('#modal_form_edit_rooster').modal('hide');
                reload_table();
            }
            else
            {
...
/*untuk menampilkan data edit rooster*/
function absensi_edit(id)
{
    $('#modal_form_edit_rooster_absensi').modal('show'); // show bootstrap modal
    $('.modal-title').text('Edit absensi Rooster'); // Set Title to Bootstrap modal title
    //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('production/rooster_edit')?>/" + id,
        type: "GET",
        ...
        {
            //===== left side is input name and then right side is field name
            $('[name="f_id"]').val(data.id_rooster_sk);
            $('[name="f_empcode"]').val(data.empcode).trigger('change');
            $('[name="f_period"]').val(data.period).trigger('change');
            $('[name="f_year"]').val(data.year).trigger('change');
            $('[name="f_begin_work"]').val(data.code_begin).trigger('change');
...
function save_abs_rooster_edit()
{
    //alert("god damn it");
    $('#f_update_by_absensi').val('<?php echo $username; ?>');
    time_stamp_update();
    var formData = new FormData($('#form_edit_rooster_absensi')[0]);
    $.ajax({
        url: "<?php echo site_url('production/update_rooster_absensi') ?>",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
        ...
        success: function(data)
        {
            if(data.status) //if success close modal and reload ajax table
            {
                $('#modal_form_edit_rooster_absensi').modal('hide');
                reload_table();
            }
            else
            {
...
function time_stamp_update()
{
    /**/
    var time_stamp = $('#f_update_date');
    var time_stamp_rooster = $('#f_update_date_rooster');
    var time_stamp_absensi = $('#f_update_date_absensi');
    $('#f_update_by').val('<?php echo $username; ?>');
    time = moment().format('YYYY-MM-DD H:mm:ss');
    time_stamp.val(time); //param untuk updatedate edit rooster dan asbesni
    time_stamp_rooster.val(time); // param untuk updatedate rooster modal
    time_stamp_absensi.val(time); //param untuk updatedate absensi
    /**/
    /* panggil fungsinya dengan time_stamp_input(); // fungsi timestamp */
}
...
<!-- modal update absensi rooster -->
<div class="modal fade" id="modal_form_edit_rooster_absensi" role="dialog" data-backdrop="static" data-keyboard="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title">Edit Absensi Rooster</h3>
            </div>
            <div class="modal-body form">
                <form action="#" id="form_edit_rooster_absensi" class="form-horizontal">
  ...
  <tbody>
    <tr>
      ...
      <table cellpadding="50" cellspacing="50">
        ...
        <input type="text" style="display:none;" name="f_update_by_absensi" id="f_update_by_absensi" value="">
        <input type="text" style="display:none;" name="f_update_date_absensi" id="f_update_date_absensi" value="">
         <input type="text" style="display:none;" name="f_id_rooster" id="f_id_rooster" value="">
                              ...
                              <input type="text" style="display:none;" name="f_id" readonly>
                              <tr>
                                <td>Nama </td>
                                <td id="td_empcode">
                                 <?php
                                  $attribute = 'class="form-control select2" id="f_empcode" style="width: 100%; display:none;" disabled';
                               ...
                               <tr>
                                <td>Period</td>
                                <td><select class="form-control select2" style="width: 100%;" name="f_period" id="f_period" disabled>
                                    <?php for ($i=1; $i <= 12 ; $i++) { 
            ?> <option value="<?php echo $i; ?>"><?php echo $i; ?></option>
                              ...
                              <tr>
                                <td>Year</td>
                                  <td>
                                      <select class="form-control select2" style="width: 100%;" id="f_year" name="f_year" disabled>
            ...
            <option value="2020">2020</option>
                              ...
                              <tr>
                                  <td>Start Date</td>
                                  <td><input type="text" name="f_start_date" id="f_start_date" class="form-control" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask > <span class="help-block"></span></td>
                              </tr>
                              <tr>
                                  <td>End Date</td>
                                  <td><input type="text" name="f_end_date" id="f_end_date" class="form-control" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask > <span class="help-block"></span></td>
                              </tr>
                              <tr>
                                <td>Begin Work From</td>
                                <td>
                                   <select class="form-control select2" style="width: 100%;" id="f_begin_work" name="f_begin_work">
            <option value="0">OFF</option>
            <option value="1">1#pagi</option>
            <option value="2">2#pagi</option>
            <option value="3">3#pagi</option>
            <option value="11">1#malam</option>
            <option value="22">2#malam</option>
            <option value="33">3#malam</option>
            <option value="10">1#reguler</option>
            <option value="20">2#reguler</option>
            <option value="30">3#reguler</option>
            <option value="40">4#reguler</option>
            <option value="50">5#reguler</option>
            <option value="60">6#reguler</option>
            <option value="100">OFF#reguler</option>
            <option value="612">6#PM</option>
            <option value="621">6#MP</option>
            <option value="601">6#OP</option>
            <option value="602">6#OM</option>
          </select><span class="help-block"></span>
                                </td>
                              </tr>    
                              <tr id="f_day_absensi">
                        ...
                        <div class="col-sm-12">
                        <input type="number" id="f_day" name="f_day" class="form-control"><span class="help-block">
                        </div>
                    </div>
                              </td>
                              </tr>                       
      ...
      <table cellpadding="50" cellspacing="50">
            ...
            <div class="modal-footer">
                <button type="button" id="btn_rooster_edit" onclick="save_abs_rooster_edit()" class="btn btn-primary">Save</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal">Cancel</button>
            </div>
  </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
<!-- End Bootstrap modal absensi rooster -->
<!-- modal update rooster -->
<div class="modal fade" id="modal_form_edit_rooster" role="dialog" data-backdrop="static" data-keyboard="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title">Edit Rooster</h3>
            </div>
            <div class="modal-body form">
                <form action="#" id="form_edit_rooster" class="form-horizontal">
                    <div class="table-responsive">
                  <table class="table table-hover">
  <thead>
  </thead>
  <tbody>
    <tr>
      ...
      <table cellpadding="50" cellspacing="50">
        <input type="text" style="display:none;" name="f_input_by" id="f_input_by" value="">
        <input type="text" style="display:none;" name="f_input_date" id="f_input_date" value="">
        <input type="text" style="display:none;" name="f_update_by_rooster" id="f_update_by_rooster" value="">
        <input type="text" style="display:none;" name="f_update_date_rooster" id="f_update_date_rooster" value="">
         <input type="text" style="display:none;" name="f_id_rooster" id="f_id_rooster" value="">
      ...
      <table cellpadding="50" cellspacing="50">
                            <tr>
                                ...
                                ?><td><?php echo "D".$i; ?></td>
                                <td>
                                   <select class="form-control select2" style="width: 100%;" id="f_day_<?php echo $i; ?>" name="f_day_<?php echo $i; ?>">
            <option value="0">OFF</option>
            ...
            <div class="modal-footer">
                <button type="button" id="btn_rooster_edit" onclick="save_rooster_edit()" class="btn btn-primary">Save</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal">Cancel</button>
            </div>
  </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
