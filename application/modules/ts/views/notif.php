...
$.ajax({
        ...
        }); // end ajax daftar kkontrak kendaraan

// ajax notif tidak p2h
$.ajax({
            url : "<?php echo site_url('vehicle/list_notif_tidak_p2h')?>",
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
              for(var i =0;i < data.length;i++)
                  {
                    var item = data[i];
                    //console.log(item.doc_type + item.valid_until + item.nap + item.code_unit);
                    $('#list_notif_ga').append('<a href= "<?php echo base_url('vehicle/vehicle_monitoring_p2h'); ?>"><i class="fa fa fa-question text-red"></i><b>' + item.nap +'</b>  <b> '
                      + item.nomor_plat + ' </b>' + item.pic + ' Terakhir P2H tanggal <b>' + item.last_date+'</b>, KM terakhir '+ item.last_km+'</a>')
...
// ajax notif next service
$.ajax({
            url : "<?php echo site_url('vehicle/list_notif_next_service')?>",
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
              for(var i =0;i < data.length;i++)
                  {
                    var item = data[i];
                    //console.log(item.doc_type + item.valid_until + item.nap + item.code_unit);
                    $('#list_notif_ga').append('<a href= "<?php echo base_url('vehicle/vehicle_next_service'); ?>"><i class="fa fa fa-automobile text-red"></i><b>' + item.nap +'</b>  <b> '
                      + item.code_unit + ' </b> sudah mendekati next service <b>' + item.next_service+'</b>, KM terakhir '+ item.max_km+'</a>')
