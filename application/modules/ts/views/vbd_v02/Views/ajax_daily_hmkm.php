...
function edit_emp_attribute(id)
{
    ...
    //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('ts/edit_vehicle_daily')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="f_year"]').val(data.year);
            $('[name="f_period"]').val(data.period);
            $('[name="f_id_hm"]').val(data.id_hm);
            $('#f_d1').val(data.d1);
            $('#f_d2').val(data.d2);
            $('#f_d3').val(data.d3);
            $('#f_d4').val(data.d4);
  ...
  if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('ts/delete_daily_hm')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function sav()
{
    ...
    if (save_method == 'add') {
        ...
        time_stamp_input();
        url = "<?php echo site_url('ts/veh_daily_hm_add') ?>";
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
